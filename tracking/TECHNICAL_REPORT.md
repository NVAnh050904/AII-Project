...
### 4.7. [Stage 3g] Mở rộng Tập Test lên 4 Video ($N = 12$ Sự kiện Độc lập)
Để tăng độ tin cậy thống kê, dự án mở rộng thu thập 4 video test từ các nguồn benchmark công khai:
1. `store-aisle-detection.mp4` ($N_{\text{pos}} = 4$ events, $N_{\text{neg}} = 15$ pairs)
2. `people-detection.mp4` ($N_{\text{pos}} = 1$ event, $N_{\text{neg}} = 15$ pairs)
3. `person-bicycle-car-detection.mp4` ($N_{\text{pos}} = 1$ event, $N_{\text{neg}} = 3$ pairs)
4. `vtest.avi` ($N_{\text{pos}} = 6$ events, $N_{\text{neg}} = 276$ pairs)

* **Tổng cỡ mẫu**: $N = 12$ sự kiện re-entry độc lập và 309 cặp negative khác người.
* **Kết quả**: Khoảng tin cậy 95% CI thu hẹp rõ rệt từ `[22.5%, 25.8%]` xuống **[9.34%, 16.59%]**.

### 4.8. [Stage 3h] Phân tích Lệch trọng số Domain (Micro EER vs Macro EER)
Khi gộp dữ liệu 4 video, kiểm toán phát hiện video `vtest.avi` có 24 tracks, tạo ra $C(24, 2) = 276$ cặp negative, **chiếm 89.3% tổng số cặp negative của toàn bộ nghiên cứu**.

Do đó, báo cáo tiến hành đo đạc song song 2 góc nhìn:
* **Micro EER (Pair-Weighted)**: Đánh giá trọng số đồng đều trên từng cặp. Đạt **9.67%** tại ngưỡng $T = 0.612$.
* **Macro EER (Video-Weighted Average)**: Tính EER riêng cho từng video có đủ dữ liệu ($N_{\text{pos}} \ge 3$) rồi lấy trung bình:
  ...
  * EER `vtest.avi`: 4.89%
  * $\text{Macro EER} = \frac{22.50\% + 4.89\%}{2} =$ **13.70%**.
...
| **Stage 3g-3h** | Multi-video Micro (Gộp 4 video) | 12 events | 309 pairs | **9.67%** | [9.34%, 16.59%] | Ngưỡng optimal $T = 0.612$; $vtest$ chiếm 89% negative |
| **Stage 3h** | Multi-video Macro (Trung bình video) | 12 events | 309 pairs | **13.70%** | N/A | **Con số đại diện khách quan nhất** cho đa môi trường |
